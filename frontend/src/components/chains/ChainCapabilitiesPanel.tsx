...
export function ChainCapabilitiesPanel() {
  ...
  if (rows.length === 0) return <div className="text-sm text-white/50">{t('No chains available.')}</div>;
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="text-left text-white/50">
            <th className="py-2 pr-4">{t('Chain')}</th>
            {CAPS.map((c) => (
            ...
            <tr key={c.name} className="border-t border-white/10">
              ...
              {CAPS.map((cap) => (
                <td key={cap.key} className="px-2 text-center">
                  {c[cap.key] ? (
                    <span className="text-emerald-400" aria-label={t('supported')}>✓</span>
                  ) : (
                    <span className="text-white/20" aria-label={t('not supported')}>—</span>
                  )}
                </td>
              ))}
            </tr>
      ...
      <p className="mt-2 text-[11px] text-white/30">
        {t('Honest support frontier — a ✓ means that capability is actually wired for the chain, not just "enabled". Execution-side capabilities on Flare depend on FLARE_DEFI_ENABLED.')}
      </p>
    </div>
  );
}
