-- S2: DeFi Discovery — WalletWatchlist + DeFiPosition + ProtocolContract + DeFiInteraction
-- Additive only. All statements use IF NOT EXISTS — safe to re-run.

-- CreateTable: wallet_watchlist (required by defi_positions and defi_interactions FKs)
...
-- CreateTable: defi_positions (created here with all columns including confidence fields)
...
-- FK: defi_positions -> wallet_watchlist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
...
-- Add confidenceLevel and sourceProvider to defi_positions if they somehow don't exist yet
-- (safe no-op when the table was just created above with these columns already)
ALTER TABLE "defi_positions"
...
-- FK: defi_interactions -> wallet_watchlist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
