...
- CONSEJO: eliges 1–32 firmantes con pesos y un quórum. Para familia, un punto de partida sólido es 5 firmantes con quórum 3 (sobrevive a perder 2 llaves). Este paso —y SOLO este— se hace FUERA de Astryum, en el xApp Multisign de Xaman, desde el móvil que guarda la llave maestra de la cuenta: Xaman no permite que ninguna app componga un SignerListSet (rechaza la petición con el error 1217). Astryum prepara la lista, la revisa (una dirección con una letra mal, o un quórum mayor que el total, bloquean la cuenta para siempre), guía paso a paso la pantalla del xApp y después lee el ledger para confirmar que el consejo creado es el que se planeó. Todo lo demás vuelve a Astryum.
